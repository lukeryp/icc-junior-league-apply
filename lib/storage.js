...
// Max JSON.parse iterations to handle Upstash double-encoding quirk
const JSON_UNWRAP_DEPTH = 3;
...
async function redisGet(key) {
  ...
  // Multi-pass unwrap: handles both old double-encoded data and new single-encoded data
  let val = data.result;
  for (let i = 0; i < JSON_UNWRAP_DEPTH && typeof val === 'string'; i++) {
...
export async function deleteSubmission(email) {
  const normalEmail = email.toLowerCase().trim();
  if (hasRedis()) {
    const all = (await redisGet(KV_KEY)) || [];
    const next = all.filter(s => s.email !== normalEmail);
    if (next.length === all.length) return false;
    await redisSet(KV_KEY, next);
    return true;
  }
  if (process.env.NODE_ENV === 'development') {
